...
export default function WorkspacePage() {
  ...
  const documentUrl = task.document?.storageUrl || "";
  ...
  const isImage =
    ...
    /\.(png|jpe?g|webp|gif|tiff?)($|[?#])/i.test(rawUrl);
  const isDocx =
    rawFileType.includes("wordprocessingml") ||
    rawFileType.includes("msword") ||
    rawName.endsWith(".docx") ||
    rawName.endsWith(".doc");
  const inlineViewerUrl = isDocx
    ...
    <div className="fixed inset-0 bg-slate-100 flex flex-col select-none">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-1/2 bg-slate-200 flex flex-col border-r border-slate-300">
          ...
          <div className="flex-1 overflow-auto p-4 flex items-start justify-center bg-slate-100">
            {documentUrl ? (
              <div 
                className="flex items-center justify-center transition-all min-h-full"
              ...
              >
                {previewFailed ? (
                  ...
                  <iframe
                    src={inlineViewerUrl}
                    title={task.document?.name || "Document preview"}
                  ...
                  <div className="w-[450px] bg-white p-8 border border-slate-200 rounded-xl text-center shadow-lg">
                    <FileText className="w-12 h-12 text-blue-600 mx-auto mb-3" />
                    <h4 className="font-semibold text-slate-800 text-sm mb-1">Preview Unavailable</h4>
                    <p className="text-xs text-slate-400 mb-4">
                      This file type cannot be rendered inline by the browser.
                    </p>
                    <a
                      href={documentUrl}
                      target="_blank"
